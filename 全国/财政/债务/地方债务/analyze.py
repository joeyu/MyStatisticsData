...
ax = df_balance.plot.bar(ax = axes[0], title = '地方政府债务余额', ylabel = '账务余额（元）')
...
df_balance_pct.plot(ax = axes[1], grid = True, title = '地方政府债务余额年增长率', ylabel = '年增长率（%）')
#ax.set_ylabel("年增长率（%）")
...
df_cap = df_liability / ( df_gpbr + df_gf)
ax = df_cap.loc['2020'].plot.bar(ax = ax, title = '2020年地方政府偿债能力 —— 债务余额 / (一般公共预算收入 + 政府性基金收入)')
for container in ax.containers:
    ax.bar_label(container)
